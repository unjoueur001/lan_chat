...
class LANchat:
    ...
    def _rafraichir_stats_droite(self):
        ...
        niv, restant = niveau_from_xp(self.compte["xp"]) if self.compte else (0, 0)
        nb_part = len(self.profils) if self.profils else 0
        nb_amis = len(self.compte.get("amis", [])) if self.compte else 0
        txt = f"XP: {self.compte['xp'] if self.compte else 0}\nNiveau: {niv}\nCoins: {self.compte['coins'] if self.compte else 0}\nMessages: {self.compte.get('msgs', 0) if self.compte else 0}\nAmis: {nb_amis}\nConnectés: {nb_part}"
