...
class LANchat:
    ...
    def _btn_styler(self, b, bg=C_BLUE, fg=C_BG):
        b.config(font=(FONT, 11, "bold"), bg=bg, fg=fg, relief="flat", cursor="hand2",
                 ...
                 padx=16, pady=8, overrelief="flat")
        orig_bg = bg; hover_bg = self._lighten(bg, 0.35); press_bg = self._darken(bg, 0.2)
        glow_bg = self._lighten(bg, 0.55)
        def on_enter(e):
            b.config(bg=hover_bg)
            try:
                steps = 6; cur = [0]
                def fade():
                    if not b.winfo_exists() or cur[0] >= steps: return
                    mix = orig_bg if cur[0] < steps/2 else hover_bg
                    t = cur[0] / steps
                    r1,g1,bl1 = hex_to_rgb(orig_bg); r2,g2,bl2 = hex_to_rgb(hover_bg)
                    nr = int(r1+(r2-r1)*t); ng = int(g1+(g2-g1)*t); nb = int(bl1+(bl2-bl1)*t)
                    b.config(bg=rgb_to_hex(nr,ng,nb)); cur[0] += 1
                    self.root.after(12, fade)
                fade()
            except Exception: b.config(bg=hover_bg)
        def on_leave(e):
            try:
                steps = 6; cur = [0]
                def fade():
                    if not b.winfo_exists() or cur[0] >= steps: return
                    t = cur[0] / steps
                    r1,g1,bl1 = hex_to_rgb(hover_bg); r2,g2,bl2 = hex_to_rgb(orig_bg)
                    nr = int(r1+(r2-r1)*t); ng = int(g1+(g2-g1)*t); nb = int(bl1+(bl2-bl1)*t)
                    b.config(bg=rgb_to_hex(nr,ng,nb)); cur[0] += 1
                    self.root.after(12, fade)
                fade()
            except Exception: b.config(bg=orig_bg)
        def on_press(e):
            b.config(bg=press_bg, relief="flat")
        def on_release(e):
            b.config(bg=hover_bg, relief="flat")
    ...
    def _ecran_demarrage(self):
        ...
        self._particules(f, count=10, height=110)
        ...
        tk.Label(conteneur, text=VERSION, font=(FONT, 9, "bold"), fg=C_YELLOW, bg=C_BG).pack(pady=(0, 8))
        ...
        tk.Label(barre, text=f"{avatar} {self.pseudo}  {badge_emoji(badge)}", font=(FONT, 12, "bold"),
            fg=couleur_hex(self.compte["equip"]["couleur"]), bg=C_SURF2).pack(side="left", padx=14, pady=10)
        self.lbl_compte = tk.Label(barre, text="", font=(FONT, 9), fg=C_SUB, bg=C_SURF2)
        self.lbl_compte.pack(side="right", padx=14, pady=10); self._rafraichir_compte_ui()
        ...
        self._btn_styler(b1, C_BLUE); b1.pack(padx=16, pady=(16, 5), fill="x")
        b2 = tk.Button(cadre, text="🔍  Rejoindre (auto)", command=self.demarrer_rejoindre, height=2)
        self._btn_styler(b2, C_GREEN); b2.pack(padx=16, pady=5, fill="x")
        b3 = tk.Button(cadre, text="✍️  Rejoindre par IP (dernier recours)", command=self.rejoindre_ip_manuel, height=2)
        self._btn_styler(b3, C_INP, C_TEXT); b3.pack(padx=16, pady=5, fill="x")
        b4 = tk.Button(cadre, text="🛍️  Boutique & cosmétiques", command=self.ouvrir_boutique, height=2)
        self._btn_styler(b4, C_YELLOW, C_BG); b4.pack(padx=16, pady=5, fill="x")
        b_ami = tk.Button(cadre, text="👥  Mes amis", command=self.ouvrir_amis, height=2)
        self._btn_styler(b_ami, C_PINK); b_ami.pack(padx=16, pady=5, fill="x")
        b5 = tk.Button(cadre, text="⚙️  Paramètres", command=self.ouvrir_parametres, height=2)
        self._btn_styler(b5, C_PURPLE); b5.pack(padx=16, pady=5, fill="x")
        b7 = tk.Button(cadre, text="🚪  Changer de compte", command=self._deconnexion, height=2)
        self._btn_styler(b7, C_RED, C_BG); b7.pack(padx=16, pady=(5, 16), fill="x")
        tk.Label(conteneur, text="Astuce : l'hébergeur lance le salon, les autres le rejoignent automatiquement.",
            font=(FONT, 9), fg=C_DIM, bg=C_BG, wraplength=440, justify="center").pack(pady=(6, 12))
