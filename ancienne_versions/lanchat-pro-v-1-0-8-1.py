...
class GestionnaireComptes:
    ...
    def creer(self, pseudo, mdp):
        ...
        self.comptes[pseudo] = {"salt":salt, "hash":self._hash(mdp, salt), "coins":150, "xp":0,
            ...
            "avatar_emoji":"🧑", "couleur_pseudo":"bleu", "amis":[], "avatar_pref":AVATAR_DEFAUT, "cadre":CADRE_DEFAUT, "fond":"defaut", "possedes_avatars":[AVATAR_DEFAUT], "possedes_cadres":[CADRE_DEFAUT], "possedes_fonds":["defaut"]}}
...
class LANchat:
    ...
    def _ecran_demarrage(self):
        ...
        b_ami = tk.Button(cadre, text="👥  Mes amis", command=self.ouvrir_amis, height=2)
    ...
    def ouvrir_journal(self):
        ...
        versions = [
            ("V.1.0.8.1", "Correctifs de stabilité", [
                "Correction des f-strings imbriqués causant un crash au démarrage",
                "Déclaration d'encodage UTF-8 explicite ajoutée",
                "Gestion d'erreurs au démarrage avec affichage du traceback",
                "Retrait du paramètre bg=parent[\"bg\"] sur les Canvas (compatibilité)",
            ]),
            ...
            ("V.1.0.7", "Sécurité, slots, amis & émojis", [
                ...
                "Système d'amis : ajouter/supprimer depuis le profil ou le menu",
                "Liste d'amis avec statut en ligne/hors ligne et accès MP",
                "Bouton Amis dans le menu principal et la barre de chat",
                "Sélecteur d'émojis refait : barre de défilement, molette, recherche globale",
                "Catégories d'émojis scrollables horizontalement (28 catégories)",
                "Plus d'émojis : 8 nouvelles catégories (Maison, Santé, Vêtements, Personnages, Drapeaux+, Temps, Argent)",
                "Total : 28 catégories d'émojis avec 24 émojis chacune (672 émojis !)",
                "Recherche d'émojis par mot-clé dans toutes les catégories",
                "Notification réseau quand un membre quitte un salon",
                "Menu principal enrichi : bouton Amis ajouté",
            ]),
    ...
    def _construire_interface_chat(self):
        ...
        b4_chat = tk.Button(barre_saisie, text="👥", command=self.ouvrir_amis, padx=10)
    ...
    def _ouvrir_carte_profil(self, pseudo):
        ...
        tk.Label(corps, text=f"🎨 Couleur : {coul_nom}\n"
            f"🏅 Badge : {badge_nom}\n"
            f"🏷️ Titre : {titre_nom_aff}",
        ...
        if pseudo == self.pseudo:
            ...
            if self._est_ami(pseudo):
                b3 = tk.Button(btns, text="✅ Ami", command=lambda: (self._supprimer_ami(pseudo), self._rafraichir_carte_ami(win, pseudo)))
                self._btn_styler(b3, C_PINK); b3.pack(side="left", padx=6)
            else:
                b3 = tk.Button(btns, text="➕ Ami", command=lambda: (self._ajouter_ami(pseudo), self._rafraichir_carte_ami(win, pseudo)))
    ...
    def _ajouter_ami(self, pseudo):
        if pseudo == self.pseudo:
            messagebox.showwarning(APP_NAME, "Tu ne peux pas t'ajouter toi-même comme ami !"); return
        if pseudo in self.compte.get("amis", []):
            messagebox.showinfo(APP_NAME, f"{pseudo} est déjà ton ami."); return
        amis = self.compte.get("amis", [])
        amis.append(pseudo); self.compte["amis"] = amis; self.gc.sauver()
        self._toast("Ami ajouté", f"{pseudo} a été ajouté à tes amis.")

    def _supprimer_ami(self, pseudo):
        amis = self.compte.get("amis", [])
        if pseudo in amis:
            amis.remove(pseudo); self.compte["amis"] = amis; self.gc.sauver()
            self._toast("Ami retiré", f"{pseudo} a été retiré de tes amis.")

    def _est_ami(self, pseudo):
        return pseudo in self.compte.get("amis", [])

    def ouvrir_amis(self):
        win = tk.Toplevel(self.root); win.title("👥 Mes amis"); win.geometry("480x560")
        win.configure(bg=C_BG); win.transient(self.root); win.grab_set()
        self._fond_gradient(win, height=40)
        ...
        if not amis:
            ...
            for ami in sorted(amis):
                ...
                b_suppr = tk.Button(row, text="🗑️", font=(FONT, 9), bg=C_RED, fg=C_BG, relief="flat", bd=0, cursor="hand2",
    ...
    def ouvrir_selecteur_emoji(self):
        win = tk.Toplevel(self.root); win.title("😀 Sélecteur d'émojis"); win.geometry("560x620")
        win.configure(bg=C_BG); win.transient(self.root); win.grab_set()

        # --- Barre de recherche ---
        barre_rech = tk.Frame(win, bg=C_BG2); barre_rech.pack(fill="x", padx=8, pady=(8, 4))
        tk.Label(barre_rech, text="🔍", font=(FONT, 12), bg=C_BG2).pack(side="left", padx=4)
        ...
        # --- Barre des catégories (scrollable horizontalement) ---
        ...
        # --- Zone d'émojis scrollable (Canvas + Scrollbar vertical) ---
        ...
        btn_import = tk.Button(barre_perso, text="📁 Importer émoji / GIF", command=self._importer_emoji)
        ...
        def _afficher_emojis(cat, filtre):
            ...
            if cat == "Perso" and persos:
                if PIL_OK:
                    ...
                    tk.Label(conteneur, text="Installe Pillow pour les émojis persos", font=(FONT, 10), fg=C_DIM, bg=C_BG).pack(pady=20)
            ...
            lbl_info.config(text=f"{len(liste)} émoji(s)")
            ...
            if filtre:
                ...
                for i, emoji in enumerate(tous):
                    btn = tk.Button(conteneur, text=emoji, font=(FONT, 22), bg=C_SURF, fg=C_TEXT,
                        relief="flat", bd=0, cursor="hand2", command=lambda e=emoji: inserer(e))
                    btn.grid(row=i // nb_col, column=i % nb_col, padx=3, pady=3)
                lbl_info.config(text=f"{len(tous)} résultat(s)")
            else:
                _afficher_emojis(cat, "")
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            _mb.showerror("LANchat Pro - Erreur",
                "Erreur au demarrage:\n\n" + traceback.format_exc())
            _root.destroy()
        except Exception:
            input("Erreur fatale. Appuyez sur Entree pour fermer.\n")