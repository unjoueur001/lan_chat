#!/usr/bin/env python3
import os, json, socket, threading, queue, random, time, hashlib, logging
...
class LANchat:
    def __init__(self):
        ...
        self._anim_pseudos={}; self._anim_callbacks=[]
    ...
    def _check_maj(self):
        v,url=verifier_maj()
        if v and v!=VERSION:
            self.ui_queue.put(("toast",f"🔄 Maj dispo: {v}",url or None))
    ...
    def _ecran_demarrage(self):
        ...
        tk.Label(barre,text=f"👤 {self.pseudo}  {badge_emoji(badge)}",font=(FONT,11,"bold"),
        ...
        self._btn_styler(b4,C_YELLOW,C_BG); b4.pack(padx=16,pady=(6,16),fill="x")
    ...
    def _gerer_client(self,conn,addr):
        ...
        if pseudo_client:
            ...
            self._diffuser(make_msg("sys",text=f"🔴 {pseudo_client} a quitté"),except_=conn)
    ...
    def _on_mp_recu(self,src,texte):
        ...
        beep(1000,100); self._toast(f"✉ MP de {src} : {texte[:30]}",lambda:self._ouvrir_fenetre_mp(src))
    ...
    def _ouvrir_fenetre_appel(self,peer,appelant=False):
        ...
        self.lbl_appel_statut=tk.Label(win,text="En attente…",font=(FONT,11),fg=C_SUB,bg=C_BG)
    ...
    def _on_appel_entrant(self,src):
        ...
        win.configure(bg=C_BG); win.transient(self.root); win.grab_set()
        tk.Label(win,text="📞",font=(FONT,36),bg=C_BG,fg=C_YELLOW).pack(pady=(16,2))
        ...
        b1=tk.Button(btns,text="✓ Accepter",command=accepter)
        self._btn_styler(b1,C_GREEN); b1.pack(side="left",padx=8)
        b2=tk.Button(btns,text="✗ Refuser",command=refuser)
    ...
    def _on_appel_ready(self,peer,audio_ip,audio_port):
        ...
        if win and win.winfo_exists(): win.lbl_appel_statut.config(text="Connecté !")
    ...
    def _on_appel_refuse(self,peer):
        ...
        if win and win.winfo_exists():
            win.lbl_appel_statut.config(text="Appel refusé 😕"); self.root.after(2500,win.destroy)
    ...
    def _on_appel_fin(self,peer):
        ...
        win=self.fenetres_appel.pop(peer,None)
        if win and win.winfo_exists():
            win.lbl_appel_statut.config(text="Appel terminé"); self.root.after(2000,win.destroy)
