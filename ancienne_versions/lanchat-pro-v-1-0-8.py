...
class LANchat:
    ...
    def _cadre_arrondi(self, parent, bg=None, radius=12, padx=1, pady=1, **kw):
        bg = bg or C_SURF2
        canvas = tk.Canvas(parent, highlightthickness=0, bd=0, bg=parent["bg"])
    ...
    def _fond_anime(self, parent, height=80):
        canvas = tk.Canvas(parent, height=height, highlightthickness=0, bd=0, bg=parent["bg"])
        canvas.pack(fill="x")
    ...
    def _ouvrir_carte_profil(self, pseudo):
        ...
        tk.Label(corps, text=f"Avatar: {AVATARS_PREF.get(prof.get('avatar_pref',AVATAR_DEFAUT),{}).get('nom','?')}  |  Cadre: {CADRES.get(prof.get('cadre',CADRE_DEFAUT),{}).get('nom','?')}  |  Fond: {FONDS_PROFIL.get(prof.get('fond','defaut'),{}).get('nom','?')}", font=(FONT, 9), fg=C_SUB, bg=C_BG, justify="left").pack(anchor="w", pady=(4, 0))
