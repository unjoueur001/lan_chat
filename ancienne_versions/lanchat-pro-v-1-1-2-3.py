...
class LANchat:
    ...
    def _param_page_journal(self, parent):
        ...
        versions = [
            ("V.1.1.2.3", "Bouton Badges direct + envoi de message reparable", [
                "Ajout d'un bouton Badges directement dans la barre de saisie du chat (acces immediat au catalogue)",
                "Reecriture de envoyer : le message s'affiche toujours localement, aucune erreur (reseau, compte, son) ne le bloque",
                "Protections try/except sur l'affichage, l'envoi reseau, le gain d'XP et le son d'envoi",
                "Verification de l'existence du widget d'entree avant l'envoi",
            ]),
    ...
    def _construire_interface_chat(self):
        ...
        b5_badges = tk.Button(barre_saisie, text="Badges", command=self._ouvrir_catalogue_badges, padx=10)
        self._btn_styler(b5_badges, C_YELLOW, C_BG); b5_badges.pack(side="left", padx=(8, 0), ipady=4)
    ...
    def envoyer(self):
        """Envoie le message saisi dans l'entree au reseau et l'affiche localement.
        Aucune erreur (reseau, compte, son) ne doit empecher l'affichage du message."""
        if not hasattr(self, "entree") or not self.entree.winfo_exists():
            return
        ...
        if len(texte) > 500:
            texte = texte[:500]
        # 1. Afficher le message localement en priorite (jamais bloque par le reseau)
        try:
            self.afficher_message(self.pseudo, texte, moi=True)
        except Exception as _e:
            log.warning(f"envoyer: affichage local impossible: {_e}")
        # 2. Envoi reseau (tolere les erreurs : le message reste affiche localement)
        try:
            ...
            self.afficher_erreur("Connexion perdue : message affiche localement uniquement.")
        except Exception as _e:
            log.warning(f"envoyer: envoi reseau impossible: {_e}")
        # 3. Gain XP / coins (tolere les erreurs)
        try:
            self._gagner_xp(1, coins=1, msgs=1)
        except Exception as _e:
            log.warning(f"envoyer: gain xp impossible: {_e}")
        # 4. Son d envoi (tolere les erreurs)
        try:
